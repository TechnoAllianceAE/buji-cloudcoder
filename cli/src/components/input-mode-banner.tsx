...
> = {
  ...
  ...(IS_BCP ? {} : { usage: ({ showTime }: { showTime: number }) => <UsageBanner showTime={showTime} /> }),
  ...(IS_BCP ? {} : { referral: () => <ReferralBanner /> }),
  help: () => <HelpBanner />,
  ...(CLAUDE_OAUTH_ENABLED && !IS_BCP
    ? { 'connect:claude': () => <ClaudeConnectBanner /> }
    : {}),
  ...(IS_BCP ? {} : { subscriptionLimit: () => <SubscriptionLimitBanner /> }),
