...
export const LoginModal = ({
  ...
  const fetchLoginUrlAndOpenBrowser = useCallback(async () => {
    ...
    fetchLoginUrlMutation.mutate(undefined as any, {
      onSettled: () => {
        setLoading(false)
      },
    })
  ...
  const handleLoginSuccess = useCallback((token: string) => {
    // Construct a partial user with the token, useLoginMutation will fetch the rest
    ...
    loginMutationRef.current.mutate(partialUser, {
      ...
      onError: (error) => {
        logger.error(
          ...
          '❌ Login validation failed, proceeding with raw token',
        )
        onLoginSuccessRef.current(partialUser)
      },
    })
  }, [])
            ...
            >
              ...
              {isRemoteSession() && !isVerySmall && (
                <text style={{ wrapMode: 'word' }}>
                  ...
                  <span fg={theme.primary}>{'bcp'} login</span>
                  <span fg={theme.secondary}>
                    {' '}instead.
                  </span>
                </text>
