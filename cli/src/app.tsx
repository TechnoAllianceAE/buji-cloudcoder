import { isRetryableStatusCode, getErrorStatusCode } from '@bcp/sdk'
  ...
  const headerContent = useMemo(() => {
    ...
    return (
      ...
      >
        ...
        >
          BujiCoderPlus will run commands on your behalf to help you build.
        </text>
        <text
          style={{ wrapMode: 'word', marginBottom: 0, fg: theme.foreground }}
        >
          Directory{' '}
          <TerminalLink
            text={displayPath}
        ...
        <text style={{ fg: theme.muted, marginBottom: 1 }}>
          {`v${process.env.BCP_CLI_VERSION || '0.0.0'} (${process.env.BCP_BUILD_HASH || 'dev'}) built ${process.env.BCP_BUILD_TIME || 'unknown'}`}
        </text>
      </box>
    )
