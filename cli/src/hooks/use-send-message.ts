...
import { getCodebuffClient } from '../utils/bcp-client'
import { AGENT_MODE_TO_ID, AGENT_MODE_TO_COST_MODE, IS_BCP } from '../utils/constants'
...
import type { AgentDefinition, MessageContent, RunState } from '@bcp/sdk'
...
export const useSendMessage = ({
  ...
  const sendMessage = useCallback<SendMessageFn>(
    async ({ content, agentMode, postUserMessage, attachments }) => {
      ...
      logger.info({}, '[send-message] Step 1: Starting validation')
      try {
        const validationResult = await onBeforeMessageSend()
        logger.info({ success: validationResult.success }, '[send-message] Step 2: Validation complete')
      ...
      // Get SDK client
      logger.info({}, '[send-message] Step 3: Getting SDK client')
      const client = await getCodebuffClient()
      logger.info({ hasClient: !!client }, '[send-message] Step 4: SDK client obtained')

      if (!client) {
        logger.error(
          {},
          '[send-message] No BCP client available. Please ensure you are authenticated.',
        )
      ...
      try {
        ...
        logger.info({}, '[send-message] Step 5: Calling client.run()')
        const runState = await client.run(runConfig)
        logger.info({}, '[send-message] Step 6: client.run() completed')
