import { createMockLogger } from '@bcp/common/testing/mocks/logger'
import { describe, test, expect, beforeEach, afterEach, mock } from '@bcp/common/testing/vitest-compat'
...
describe('fetchUsageData', () => {
  ...
  const originalEnv = process.env.NEXT_PUBLIC_BCP_APP_URL

  beforeEach(() => {
    process.env.NEXT_PUBLIC_BCP_APP_URL = 'https://test.bcp.local'
  })

  afterEach(() => {
    globalThis.fetch = originalFetch
    process.env.NEXT_PUBLIC_BCP_APP_URL = originalEnv
    mock.restore()
  })
  ...
  test('should throw error when app URL is not set', async () => {
    await expect(
      fetchUsageData({
        ...
        clientEnv: {
          NEXT_PUBLIC_BCP_APP_URL: undefined,
        } as unknown as ClientEnv,
      }),
    ).rejects.toThrow('NEXT_PUBLIC_BCP_APP_URL is not set')
  })
  ...
  test('should call correct API endpoint', async () => {
    ...
    await fetchUsageData({
      authToken: 'test-token',
      clientEnv: {
        NEXT_PUBLIC_BCP_APP_URL: 'https://test.bcp.local',
      } as ClientEnv,
    })

    expect(capturedUrl).toBe('https://test.bcp.local/api/v1/usage')
  })
