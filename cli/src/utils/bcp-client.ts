import { API_KEY_ENV_VAR } from '@bcp/common/old-constants'
import { AskUserBridge } from '@bcp/common/utils/ask-user-bridge'
...
let clientInstance: BCPClient | null = null
...
/**
 * Reset the cached BCPClient instance.
 * This should be called after login to ensure the client is re-initialized with new credentials.
 */
export function resetBCPClient(): void {
  clientInstance = null
}

/** @deprecated Use resetBCPClient instead */
export const resetCodebuffClient = resetBCPClient

export async function getBCPClient(): Promise<BCPClient | null> {
  if (!clientInstance) {
    ...
    if (env.BCP_IS_BINARY) {
      try {
        ...
        getSystemProcessEnv().BCP_RG_PATH = rgPath
      } catch (error) {
    ...
    try {
      ...
      logger.error(error, 'Failed to initialize BCPClient')
      return null
    }
  }
...
/** @deprecated Use getBCPClient instead */
export const getCodebuffClient = getBCPClient
