...
import type { ChatGptOAuthCredentials } from '@bcp/sdk'
...
function callbackPageHtml(success: boolean, errorMessage?: string): string {
  const title = success ? 'Connected — BujiCoderPlus' : 'Connection Failed — BujiCoderPlus'
  const heading = success ? '✓ Connected to ChatGPT' : 'Connection Failed'
  const headingColor = success ? '#4ade80' : '#f87171'
  const body = success
    ? 'You can close this tab and return to BujiCoderPlus.'
    : `${escapeHtml(errorMessage ?? 'Unknown error')}. Return to BujiCoderPlus and try /connect:chatgpt again.`
  return `<!DOCTYPE html>
