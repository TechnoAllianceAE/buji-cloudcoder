...
import { describe, expect, test } from '@bcp/common/testing/vitest-compat'
...
| Jane | 25  |`
  ...
  test('renders code fence followed by text with quotes correctly', () => {
    const markdown = `\`\`\`bash
# Start using it
bcp "add a new feature to handle user authentication"
\`\`\``
    const output = renderMarkdown(markdown)
    const nodes = flattenNodes(output)
  ...
  test('renders inline code followed by quotes correctly', () => {
    const markdown = 'Use `bcp "fix bug"` to fix bugs.'
    const output = renderMarkdown(markdown)
    const nodes = flattenNodes(output)
...
\`\`\``
    ...
    expect(textContent).toContain('bcp "implement feature" --verbose')
    expect(textContent).not.toContain('```"')
  })
