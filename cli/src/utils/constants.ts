import type { ToolName } from '@bcp/sdk'
 ...
 */
export const IS_BCP = getCliEnv().BCP_MODE === 'true'

// Agent IDs that should not be rendered in the CLI UI
export const HIDDEN_AGENT_IDS = ['bcp/context-pruner'] as const
 ...
 * Maps to the same modes as the Go CLI: normal, heavy, max, plan.
 */
...
/**
 * Maps CLI agent mode to cost mode sent to the gateway.
 * Must match the gateway's model_config.yaml modes: normal, heavy, max.
 */
export const AGENT_MODE_TO_COST_MODE = {
  ...
  MAX: 'max',
  PLAN: 'normal',
} as const satisfies Record<AgentMode, 'normal' | 'heavy' | 'max' | 'experimental' | 'ask'>
