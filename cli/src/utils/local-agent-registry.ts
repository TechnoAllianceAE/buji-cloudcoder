...
import { pluralize } from '@bcp/common/util/string'
import { loadLocalAgents as sdkLoadLocalAgents, loadMCPConfigSync } from '@bcp/sdk'

import type { MCPConfig } from '@bcp/common/types/mcp'
...
export interface LocalAgentInfo {
  ...
  displayName: string
  filePath: string
  /** True if this is a bundled BCP agent (not user-created) */
  isBundled?: boolean
}
...
export const loadLocalAgents = (currentAgentMode?: AgentMode): LocalAgentInfo[] => {
  ...
  // Get bundled agents - these are the default BCP agents
...
export const loadAgentDefinitions = (): AgentDefinition[] => {
  // Start with bundled agents - these are the default BCP agents
  const bundledAgents = getBundledAgents()
