import { CHATGPT_OAUTH_ENABLED } from '@bcp/common/constants/chatgpt-oauth'
import { CLAUDE_OAUTH_ENABLED } from '@bcp/common/constants/claude-oauth'
...
import { AGENT_MODES, IS_BCP } from '../utils/constants'
...
const BCP_REMOVED_COMMANDS = new Set([
  'ads:enable',
  'ads:disable',
  'refer-friends',
  'subscribe',
...
const BCP_ONLY_COMMANDS = new Set([
  'plan',
])
...
const ALL_COMMANDS: CommandDefinition[] = [
  ...
  defineCommand({
    ...
    handler: (params) => {
      ...
      params.setMessages((prev) => [
        ...prev,
        getUserMessage(params.inputValue.trim()),
        getSystemMessage('Re-authenticating with the server...'),
      ])
      clearInput(params)
    },
  }),
  ...
  defineCommandWithArgs({
    name: 'plan',
    handler: (params, args) => {
      // In bcp mode, require ChatGPT connection
      if (IS_BCP && !getChatGptOAuthStatus().connected) {
        params.setMessages((prev) => [
          ...prev,
          getUserMessage(params.inputValue.trim()),
          getSystemMessage(
  ...
  defineCommand({
    name: 'models',
    handler: (params) => {
      const version = process.env.BCP_CLI_VERSION || 'dev'
      const hash = process.env.BCP_BUILD_HASH || 'dev'
      params.setMessages((prev) => [
        ...prev,
        getUserMessage(params.inputValue.trim()),
        getSystemMessage(
          'Model assignments are managed by the gateway.\n' +
          'The gateway resolves models based on cost mode (normal/heavy/max) and agent role.\n' +
          'Use the admin panel at https://ent.bujicoder.com to view and configure model mappings.',
        ),
      ])
      clearInput(params)
    },
  ...
  defineCommand({
    ...
    handler: (params) => {
      params.setMessages((prev) => [
        ...prev,
        getUserMessage(params.inputValue.trim()),
        getSystemMessage('Model-agent assignments refreshed from gateway.'),
      ])
      clearInput(params)
    },
  }),
  defineCommand({
    ...
    handler: (params) => {
      params.setMessages((prev) => [
        ...
        getUserMessage(params.inputValue.trim()),
        getSystemMessage(
          `Server: ${process.env.NEXT_PUBLIC_BCP_APP_URL || 'https://ent.bujicoder.com'}\n` +
          `Config: ~/.bcp/\n` +
          'Use the admin panel to manage server settings.',
        ),
      ])
      clearInput(params)
    },
  ...
  defineCommand({
    ...
    handler: (params) => {
      ...
      const target = process.env.BCP_CLI_TARGET || 'unknown'
      params.setMessages((prev) => [
        ...prev,
        getUserMessage(params.inputValue.trim()),
        getSystemMessage(
          `BujiCoderPlus v${version} (${hash})\n` +
          `Built: ${buildTime}\n` +
          `Target: ${target}\n` +
          `Server: ${process.env.NEXT_PUBLIC_BCP_APP_URL || 'https://ent.bujicoder.com'}\n` +
          `Config: ~/.bcp/`,
  ...
  defineCommand({
    ...
    handler: (params) => {
      params.setMessages((prev) => [
        ...prev,
        getUserMessage(params.inputValue.trim()),
        getSystemMessage('To update BujiCoderPlus, run: bcp update'),
      ])
      clearInput(params)
    },
  }),
...
// Always filter out removed commands — BCP never shows upstream monetization features
export const COMMAND_REGISTRY: CommandDefinition[] = ALL_COMMANDS.filter(
  (cmd) => !BCP_REMOVED_COMMANDS.has(cmd.name),
)
