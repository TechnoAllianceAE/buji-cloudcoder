  ...
  () => {
    beforeAll(async () => {
      ...
      if (!sdkBuilt) {
        ...
        console.log('🔨 Build SDK: cd sdk && pnpm run build\n')
      }
      if (tmuxAvailable && sdkBuilt) {
        ...
        await tmux(['set-environment', '-g', 'BCP_PERF_TEST', 'true'])
      }
    })
      ...
      async () => {
        ...
        try {
          ...
          await tmux([
            ...
            '-y',
            '30',
            `BCP_PERF_TEST=true tsx ${CLI_PATH}`,
          ])
...
if (!sdkBuilt) {
  describe('Re-render Performance - SDK Required', () => {
    test.skip('Build SDK: cd sdk && pnpm run build', () => {})
  })
}
