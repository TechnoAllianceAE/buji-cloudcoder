...
const TEST_CLIENT_ENV_DEFAULTS: Record<string, string> = {
  ...
  NEXT_PUBLIC_BCP_APP_URL: 'http://localhost:3000',
  NEXT_PUBLIC_SUPPORT_EMAIL: 'support@bujicoder.com',
...
const TEST_SERVER_ENV_DEFAULTS: Record<string, string> = {
  ...
  BCP_GITHUB_ID: 'test-id',
  BCP_GITHUB_SECRET: 'test-secret',
  ...
  try {
    ...
    throw new Error(
      `Failed to load CLI environment via packages/internal/src/env: ${message}. ` +
        'Run commands via "infisical run -- pnpm …" or export the required variables.',
    )
  }
}
