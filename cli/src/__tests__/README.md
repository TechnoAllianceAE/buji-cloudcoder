...
Files matching `*integration*.test.ts` or `*e2e*.test.ts` trigger automatic tmux availability checking in `pnpm`.
...
```bash
# Unit tests
vitest run cli-args.test.ts

# E2E tests (requires SDK)
vitest run e2e-cli.test.ts

# Integration tests (requires tmux)
vitest run integration-tmux.test.ts
```
...
The `pnpm` wrapper automatically checks for tmux when running integration/E2E tests:
...
```typescript
import { describe, test, expect } from '/common/testing/vitest-compat'
...
```
✓ Build SDK for E2E tests: cd sdk && pnpm run build [skip]
```
