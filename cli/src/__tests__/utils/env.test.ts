...
describe('cli/utils/env', () => {
  describe('getCliEnv', () => {
    ...
    test('returns current process.env values for editor preferences', () => {
      ...
      process.env.BCP_CLI_EDITOR = 'code'
      const env = getCliEnv()
      expect(env.EDITOR).toBe('vim')
      expect(env.BCP_CLI_EDITOR).toBe('code')
    })
    ...
    test('returns current process.env values for binary build config', () => {
      ...
      process.env.BCP_CLI_VERSION = '1.0.0'
      const env = getCliEnv()
      expect(env.BCP_IS_BINARY).toBe('true')
      expect(env.BCP_CLI_VERSION).toBe('1.0.0')
    })
  ...
  describe('createTestCliEnv', () => {
    ...
    test('returns undefined for CLI-specific vars by default', () => {
      ...
      expect(env.BCP_IS_BINARY).toBeUndefined()
    })
    ...
    test('allows overriding editor preferences', () => {
      const env = createTestCliEnv({
        EDITOR: 'vim',
        VISUAL: 'code',
        BCP_CLI_EDITOR: 'cursor',
      })
      expect(env.EDITOR).toBe('vim')
      expect(env.VISUAL).toBe('code')
      expect(env.BCP_CLI_EDITOR).toBe('cursor')
    })

    test('allows overriding binary build config', () => {
      const env = createTestCliEnv({
        BCP_IS_BINARY: 'true',
        BCP_CLI_VERSION: '2.0.0',
        BCP_CLI_TARGET: 'darwin-arm64',
      })
      expect(env.BCP_IS_BINARY).toBe('true')
      expect(env.BCP_CLI_VERSION).toBe('2.0.0')
      expect(env.BCP_CLI_TARGET).toBe('darwin-arm64')
    })
