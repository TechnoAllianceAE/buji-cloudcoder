import { getUserInfoFromApiKey } from '@bcp/sdk'
...
describe('API Integration', () => {
  ...
  const originalAppUrl = process.env.NEXT_PUBLIC_BCP_APP_URL
  ...
  beforeEach(() => {
    process.env.NEXT_PUBLIC_BCP_APP_URL = 'https://example.bcp.test'
  ...
  afterEach(() => {
    ...
    process.env.NEXT_PUBLIC_BCP_APP_URL = originalAppUrl
    mock.restore()
  })
  ...
  describe('P2: Network Error Recovery', () => {
    ...
    test('should handle DNS resolution failures gracefully', async () => {
      const fetchMock = setFetchMock(async () => {
        const error = new Error('getaddrinfo ENOTFOUND api.bcp.local')
