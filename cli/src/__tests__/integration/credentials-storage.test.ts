...
import {
  clearMockedModules,
  mockModule,
} from '@bcp/common/testing/mock-modules'
import {
  describe,
  test,
  expect,
  ...
  mock,
  spyOn,
} from '@bcp/common/testing/vitest-compat'
 ...
 * - Dev: ~/.config/bcp-dev/credentials.json (or legacy ~/.config/manicode-dev/)
 * - Prod: ~/.config/bcp/credentials.json (or legacy ~/.config/manicode/)
 *
 * Tests ensure:
...
describe('Credentials Storage Integration', () => {
  ...
  beforeEach(() => {
    ...
    tempConfigDir = fs.mkdtempSync(path.join(os.tmpdir(), 'bcp-test-'))
  ...
  describe('P0: File System Operations', () => {
    ...
    test('should use bcp-test directory in test environment', async () => {
      // Restore getConfigDir to use real implementation for this test
      mock.restore()

      await mockModule('@bcp/common/env', () => ({
        env: { NEXT_PUBLIC_CB_ENVIRONMENT: 'test' },
      }))

      // Call real getConfigDir - returns bcp-test (new) or manicode-test (legacy)
      const configDir = authModule.getConfigDir()
      expect(configDir).toMatch(/bcp-test|manicode-test/)
    })

    test('should use bcp-dev directory in development environment', async () => {
      // Restore getConfigDir to use real implementation for this test
      mock.restore()
      ...
      // Call real getConfigDir - returns bcp-dev (new) or manicode-dev (legacy)
      const configDir = authModule.getConfigDir()
      expect(configDir).toMatch(/bcp-dev|manicode-dev/)
    })

    test('should use bcp directory in production environment', async () => {
      ...
      // Call real getConfigDir - returns bcp (new) or manicode (legacy)
      const configDir = authModule.getConfigDir()
      expect(configDir).toMatch(/\/bcp$|\/manicode$/)
    })
