...
function checkGitHubToken() {
  const token = process.env.BCP_GITHUB_TOKEN
  if (!token) {
    error(
      'BCP_GITHUB_TOKEN environment variable is required but not set.\n' +
...
async function triggerWorkflow(versionType: string) {
  ...
  try {
    ...
    const triggerCmd = `curl -s -w "HTTP Status: %{http_code}" -X POST \
      ...
      https://api.github.com/repos/TechnoAllianceAE/bcp/actions/workflows/cli-release-prod.yml/dispatches \
    ...
    if (response.includes('workflow_dispatch')) {
      ...
      log(
        'Please manually trigger the workflow at: https://github.com/TechnoAllianceAE/bcp/actions/workflows/cli-release-prod.yml',
      )
    } else {
      // log(
    ...
    log(
      'You may need to trigger it manually at: https://github.com/TechnoAllianceAE/bcp/actions/workflows/cli-release-prod.yml',
    )
  }
}
...
async function main() {
  ...
  checkGitHubToken()
  log('✅ Using local BCP_GITHUB_TOKEN')
  ...
  log('')
  log('Monitor progress at: https://github.com/TechnoAllianceAE/bcp/actions')
}
