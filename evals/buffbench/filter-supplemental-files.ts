...
function setupRepoForValidation(
  ...
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'bcp-filter-'))
...
function processEvalFile(
  ...
  if (!repoUrl) {
    console.log(`No repoUrl found in eval file, skipping`)
  ...
  try {
    ...
    console.log(`Failed to clone repo ${repoUrl}: ${error}`)
    return { totalRemoved: 0, commitUpdates: [] }
  }
...
function main() {
  ...
  if (evalFiles.length === 0) {
    console.log('Usage: pnpm run filter-supplemental-files.ts [--dry-run] <eval-file.json> ...')
    console.log('')
    console.log('Options:')
    console.log('  --dry-run  Show what would be removed without modifying files')
    console.log('')
    console.log('Examples:')
    console.log('  pnpm run filter-supplemental-files.ts eval-manifold2.json')
    console.log('  pnpm run filter-supplemental-files.ts --dry-run eval-*2.json')
    process.exit(1)
  }
  ...
  for (const evalFile of evalFiles) {
    ...
    if (!fs.existsSync(evalFilePath)) {
      console.log(`File not found: ${evalFile}`)
      continue
    }
