 ...
 *
 * Usage: pnpm run evals/buffbench/analyze-task-scores.ts [--runs N] [--output FILE] [--dir DIR]
 *
 * Options:
...
Usage: pnpm run analyze-task-scores.ts [OPTIONS]
...
Examples:
  pnpm run analyze-task-scores.ts
  pnpm run analyze-task-scores.ts --runs 5
  pnpm run analyze-task-scores.ts --dir 2025-12-07T21-57_base2
  pnpm run analyze-task-scores.ts --dir 2025-12-07T21-57_base2 --threshold 7.5 --json
  pnpm run analyze-task-scores.ts --runs 10 --output task-scores.md
`)
      process.exit(0)
    }
  }
