...
export async function setupTestRepo(
  ...
  try {
    ...
    const githubToken = process.env.BCP_GITHUB_TOKEN
    ...
    if (
      ...
      console.error(
        '1. Verify BCP_GITHUB_TOKEN environment variable is set',
      )
      console.error(
      ...
      if (token) {
        ...
        console.error('BCP_GITHUB_TOKEN environment variable is not set')
      }
    }
