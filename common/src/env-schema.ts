...
export const clientEnvSchema = z.object({
  ...
  NEXT_PUBLIC_BCP_APP_URL: z.url().min(1).default('https://ent.bujicoder.com'),
  NEXT_PUBLIC_SUPPORT_EMAIL: z.email().min(1).default('support@bujicoder.com'),
  NEXT_PUBLIC_POSTHOG_API_KEY: z.string().min(1).default('disabled'),
  NEXT_PUBLIC_POSTHOG_HOST_URL: z.url().min(1).default('https://localhost'),
  NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY: z.string().min(1).default('disabled'),
  NEXT_PUBLIC_STRIPE_CUSTOMER_PORTAL: z.url().min(1).default('https://localhost'),
...
// Runtime config reads these values, so we need to reference them individually (no for-loops)
export const clientProcessEnv: ClientInput = {
  NEXT_PUBLIC_CB_ENVIRONMENT: process.env.NEXT_PUBLIC_CB_ENVIRONMENT,
  NEXT_PUBLIC_BCP_APP_URL: process.env.NEXT_PUBLIC_BCP_APP_URL,
