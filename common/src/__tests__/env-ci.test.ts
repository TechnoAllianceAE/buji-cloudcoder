...
describe('env-ci', () => {
  describe('getCiEnv', () => {
    ...
    test('returns current process.env values for BCP_API_KEY', () => {
      process.env.BCP_API_KEY = 'test-key-123'
      const env = getCiEnv()
      expect(env.BCP_API_KEY).toBe('test-key-123')
    })

    test('returns current process.env values for BCP_GITHUB_TOKEN', () => {
      process.env.BCP_GITHUB_TOKEN = 'ghp_test_token'
      const env = getCiEnv()
      expect(env.BCP_GITHUB_TOKEN).toBe('ghp_test_token')
    })
  ...
  describe('ciEnv', () => {
    ...
    test('contains expected keys', () => {
      ...
      expect('BCP_API_KEY' in ciEnv).toBe(true)
      expect('BCP_GITHUB_TOKEN' in ciEnv).toBe(true)
    })
  })
  ...
  describe('createTestCiEnv', () => {
    test('returns a CiEnv with default test values', () => {
      ...
      expect(env.BCP_API_KEY).toBe('test-api-key')
    })
    ...
    test('returns undefined for most vars by default', () => {
      ...
      expect(env.BCP_GITHUB_TOKEN).toBeUndefined()
    })
    ...
    test('allows overriding default values', () => {
      const env = createTestCiEnv({
        BCP_API_KEY: 'custom-api-key',
      })
      expect(env.BCP_API_KEY).toBe('custom-api-key')
    })
    ...
    test('allows setting all CI-related vars for CI simulation', () => {
      const env = createTestCiEnv({
        ...
        BCP_GITHUB_TOKEN: 'ghp_simulated_token',
        BCP_API_KEY: 'test-api-key-override',
      })
      expect(env.CI).toBe('true')
      expect(env.GITHUB_ACTIONS).toBe('true')
      expect(env.RENDER).toBe('true')
      expect(env.IS_PULL_REQUEST).toBe('true')
      expect(env.BCP_GITHUB_TOKEN).toBe('ghp_simulated_token')
      expect(env.BCP_API_KEY).toBe('test-api-key-override')
    })
  })
})
