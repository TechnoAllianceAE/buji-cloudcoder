...
if [[ -n "$CUSTOM_COMMAND" ]]; then
    ...
    if [[ ! -f "$BINARY_PATH" ]]; then
        output_error "Binary not found: $BINARY_PATH. Build with: cd cli && pnpm run build:binary"
        exit 1
    fi
    ...
    # Default mode - Codebuff dev server via pnpm
    CLI_CMD="cd '$PROJECT_ROOT' && pnpm --dir cli run dev 2>&1"
    CLI_MODE="dynamic"
    CLI_DISPLAY="pnpm --dir cli run dev"
fi
