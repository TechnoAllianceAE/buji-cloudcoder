...
function parseArgs(): WorktreeArgs {
  ...
  if (args.length < 3) {
    console.error(
      'Usage: tsx scripts/init-worktree.ts <worktree-name> <backend-port> <web-port>',
    )
    console.error(
      'Example: tsx scripts/init-worktree.ts feature-branch 8001 3001',
    )
...
# This file has highest precedence for this worktree's environment, overriding .env.local
PORT=${args.webPort}
NEXT_PUBLIC_WEB_PORT=${args.webPort}
NEXT_PUBLIC_BCP_APP_URL=http://localhost:${args.webPort}
`
...
# already sets NEXT_PUBLIC_BCP_APP_URL for all packages.
NEXT_PUBLIC_BCP_APP_URL=http://localhost:${args.webPort}
`
...
// Wrapper script no longer needed - pnpm handles .env.worktree loading
// function createWrapperScript(worktreePath: string): void {
//   // This function is deprecated - the pnpm wrapper now handles .env.worktree loading
// }
...
async function main(): Promise<void> {
  try {
    ...
    // Explicitly use HEAD to ensure worktree has latest tooling (pnpm, etc.)
    ...
    // Sync secrets from Infisical to .env.local before installing dependencies.
    ...
    await runCommand('pnpm', ['install'], worktreePath)
    ...
    console.log(``)
    console.log(`⚠️  Note: NEXT_PUBLIC_BCP_APP_URL is set to http://localhost:${args.webPort}`)
    console.log(`   The CLI and SDK will hit your local web server.`)
    console.log(`   To run SDK E2E tests, start the web server first: pnpm run --cwd web dev`)
  } catch (error) {
