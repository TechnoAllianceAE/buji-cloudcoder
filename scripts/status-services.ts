 ...
 *
 * Usage:
 *   pnpm run status-services    # Check if services are running
 *
...
function isDockerDbRunning(): boolean {
  try {
    const result = spawnSync('docker', ['ps', '--filter', 'name=bcp-db', '--format', '{{.Status}}'], {
      encoding: 'utf-8',
    })
...
async function main(): Promise<void> {
  ...
  if (anyRunning) {
    console.log('  To stop: pnpm run down')
  } else {
    console.log('  To start: pnpm run up')
  }
