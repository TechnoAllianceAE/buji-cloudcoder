 ...
 *   tsx scripts/test-canopywave.ts direct
 *
 *   # Test 2: Hit our chat completions endpoint (requires running web server + valid API key)
 *   BCP_API_KEY=<key> tsx scripts/test-canopywave.ts endpoint
 *
 *   # Run both tests
 *   BCP_API_KEY=<key> tsx scripts/test-canopywave.ts both
 */
...
async function testChatCompletionsEndpoint() {
  const bcpApiKey = process.env.BCP_API_KEY
  if (!bcpApiKey) {
    console.error('❌ BCP_API_KEY is not set. Pass it as an env var.')
    console.error('   Example: BCP_API_KEY=<key> tsx scripts/test-canopywave.ts endpoint')
    process.exit(1)
  }

  const appUrl = process.env.NEXT_PUBLIC_BCP_APP_URL ?? 'http://localhost:3000'
  ...
  const response = await fetch(endpoint, {
    method: 'POST',
    headers: {
      Authorization: `Bearer ${bcpApiKey}`,
      'Content-Type': 'application/json',
    },
    body: JSON.stringify({
  ...
  if (response.ok) {
    ...
    if (response.status === 400 && data.message?.includes('runId')) {
      ...
      console.log('   ℹ️  Auth failed. Make sure BCP_API_KEY is valid.')
    }
  }
  console.log()
...
async function main() {
  ...
  switch (mode) {
    ...
    default:
      ...
      console.error('Usage: tsx scripts/test-canopywave.ts [direct|endpoint|both]')
      process.exit(1)
  }
