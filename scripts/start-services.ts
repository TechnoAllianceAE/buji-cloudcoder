 ...
 *
 * Usage:
 *   pnpm run start-services    # Start services in background
 *   pnpm run start-cli         # Then start CLI in foreground
 *   pnpm run stop-services     # Stop background services
 *
 * Services started:
 ...
 *
 * Load environment variables before invoking this script when needed.
 */
...
// Get config from the current process environment.
const APP_URL = process.env.NEXT_PUBLIC_BCP_APP_URL || 'http://localhost:3000'
...
function startDb(): boolean {
  ...
  const result = spawnSync(PNPM_CMD, ['--dir', 'packages/internal', 'run', 'db:start'], {
    cwd: PROJECT_ROOT,
...
function startBackgroundServices(): ServicePids {
  ...
  const sdk = spawnBackgroundProcess('sdk', PNPM_CMD, ['--dir', 'sdk', 'run', 'build'], 'sdk.log')
  ...
  // Start web server
  const web = spawnBackgroundProcess('web', PNPM_CMD, ['--dir', 'web', 'run', 'dev'], 'web.log')
...
async function main(): Promise<void> {
  ...
  console.log(`  Stop with:  pnpm run down`)
  console.log('')
  console.log('Now run: pnpm run start-cli')
}
