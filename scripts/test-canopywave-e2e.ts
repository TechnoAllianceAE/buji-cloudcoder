...
/**
 * E2E test for CanopyWave integration via the BCP SDK.
 *
 ...
 *
 * Usage:
 *   tsx scripts/test-canopywave-e2e.ts
 */

import { BCPClient } from '@bcp/sdk'

import type { AgentDefinition } from '@bcp/sdk'
...
async function main() {
  ...
  if (!apiKey) {
    console.error('❌ BCP_API_KEY is not set.')
    console.error('   Example: BCP_API_KEY=<key> tsx scripts/test-canopywave-e2e.ts')
    process.exit(1)
  }

  console.log('🔌 CanopyWave E2E Test via BCP SDK')
