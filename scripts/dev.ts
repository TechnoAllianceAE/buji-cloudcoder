 ...
 *
 * Usage:
 *   pnpm run dev              # Start services + CLI
 *   pnpm run dev -- --debug   # Pass args to CLI
 *
 ...
 *   pnpm run stop-services    # Stop services
 */
...
function startBackgroundServices(): void {
  // Start SDK build
  const sdk = spawnBackgroundProcess(PNPM_CMD, ['--dir', 'sdk', 'run', 'build'], 'sdk.log')
  ...
  // Start web server
  const web = spawnBackgroundProcess(PNPM_CMD, ['--dir', 'web', 'run', 'dev'], 'web.log')
...
function startCli(args: string[]): Promise<number> {
  return new Promise((resolve) => {
    ...
    cliProcess = spawn(PNPM_CMD, ['--dir', 'cli', 'run', 'dev', '--', ...args], {
      cwd: PROJECT_ROOT,
      stdio: 'inherit',
      env: process.env,
    })
