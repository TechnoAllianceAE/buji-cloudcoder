 ...
 *   tsx scripts/test-fireworks.ts direct
 *
 *   # Test 2: Hit our chat completions endpoint (requires running web server + valid API key)
 *   BCP_API_KEY=<key> tsx scripts/test-fireworks.ts endpoint
 *
 *   # Run both tests
 *   BCP_API_KEY=<key> tsx scripts/test-fireworks.ts both
 */
...
async function testChatCompletionsEndpoint() {
  ...
  if (!bcpApiKey) {
    ...
    console.error('   Example: BCP_API_KEY=<key> tsx scripts/test-fireworks.ts endpoint')
    process.exit(1)
  }
...
async function main() {
  ...
  switch (mode) {
    ...
    default:
      ...
      console.error('Usage: tsx scripts/test-fireworks.ts [direct|endpoint|both]')
      process.exit(1)
  }
