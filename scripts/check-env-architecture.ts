...
const isInternalImport = (moduleSpecifier: string): boolean =>
  moduleSpecifier === '@bcp/internal' ||
  moduleSpecifier.startsWith('@bcp/internal/')
...
const ENV_PROCESS_MODULE = '@bcp/common/env-process'
const ENV_TYPES_MODULE = '@bcp/common/types/contracts/env'
const INTERNAL_ENV_MODULE = '@bcp/internal/env'
 ...
 * from @bcp/common/env-process. These files are the designated entry points for env access.
 */
...
// common: do not allow importing from @bcp/internal (layering + secret safety)
{
  ...
  for (const absolutePath of files) {
    const relativePath = normalizePath(path.relative(cwd, absolutePath))
    const content = fs.readFileSync(absolutePath, 'utf8')
    if (!content.includes('@bcp/internal')) continue
    ...
    if (internalImportLines.length > 0) {
      ...
      violations.push({
        file: relativePath,
        message: `Disallowed import from @bcp/internal${lineInfo}; common must not depend on internal (inject server env/deps instead)`,
      })
    }
  }
}

// web: prevent Client Components from importing @bcp/internal/env
{
