...
import { getBCPApiKeyFromEnv } from './env'
import { run } from './run'

import type { RunOptions, BCPClientOptions } from './run'
import type { RunState } from './run-state'

export class BCPClient {
  public options: BCPClientOptions & {
    apiKey: string
    fingerprintId: string
  }

  constructor(options: BCPClientOptions) {
    const foundApiKey = options.apiKey ?? getBCPApiKeyFromEnv()
    if (!foundApiKey) {
      throw new Error(
        `BCP API key not found. Please provide an apiKey in the constructor of BCPClient or set the ${API_KEY_ENV_VAR} environment variable.`,
      )
    }
    ...
    this.options = {
      ...
      fingerprintId: `bcp-sdk-${Math.random().toString(36).substring(2, 15)}`,
      ...options,
    }
  }

  /**
   * Run a BCP agent with the specified options.
   *
   ...
   * @param projectFiles - (Optional) All the files in your project as a plain JavaScript object. Keys should be the full path from your current directory to each file, and values should be the string contents of the file. Example: { "src/index.ts": "console.log('hi')" }. This helps BCP pick good source files for context.
   ...
   */
  public async run(
    options: RunOptions & BCPClientOptions,
  ): Promise<RunState> {
    return run({ ...this.options, ...options })
  }

  /**
   * Check connection to the BCP backend by hitting the /healthz endpoint.
   *
   * @returns Promise that resolves to true if connected, false otherwise
   */
  public async checkConnection(): Promise<boolean> {
    try {
      const response = await fetch(`${WEBSITE_URL}/healthz`, {
        method: 'GET',
...
export const CodebuffClient = BCPClient
