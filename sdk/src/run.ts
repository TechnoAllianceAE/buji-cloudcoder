...
import { callMainPrompt } from '@bcp/agent-runtime/main-prompt'
import {
  buildUserMessageContent,
  withSystemTags,
} from '@bcp/agent-runtime/util/messages'
import { MAX_AGENT_STEPS_DEFAULT } from '@bcp/common/constants/agents'
import { toOptionalFile } from '@bcp/common/constants/paths'
import {
  getMCPClient,
  listMCPTools,
  callMCPTool,
} from '@bcp/common/mcp/client'
import { toolNames } from '@bcp/common/tools/constants'
import { clientToolCallSchema } from '@bcp/common/tools/list'
import { AgentOutputSchema } from '@bcp/common/types/session-state'
import { parseApiErrorResponseBody } from '@bcp/common/util/error'
...
import type { ServerAction } from '@bcp/common/actions'
import type { AgentDefinition } from '@bcp/common/templates/initial-agents-dir/types/agent-definition'
import type {
  PublishedToolName,
  ToolName,
} from '@bcp/common/tools/constants'
import type {
  ClientToolCall,
  ClientToolName,
  CodebuffToolOutput,
...
import type { CodebuffFileSystem } from '@bcp/common/types/filesystem'
...
import type { SessionState } from '@bcp/common/types/session-state'
import type { Source } from '@bcp/common/types/source'
import type { CodebuffSpawn } from '@bcp/common/types/spawn'
...
async function runOnce({
  ...
  // Send input
  console.error('[bcp-debug] run: getUserInfoFromApiKey start')
  const userInfo = await getUserInfoFromApiKey({
    ...agentRuntimeImpl,
    apiKey,
    fields: ['id'],
  })
  console.error('[bcp-debug] run: getUserInfoFromApiKey done', userInfo ? 'ok' : 'null')
  if (!userInfo) {
  ...
  console.error('[bcp-debug] run: callMainPrompt start')
  callMainPrompt({
    ...agentRuntimeImpl,
    promptId,
    action: {
...
function requireCwd(cwd: string | undefined, toolName: string): string {
  if (!cwd) {
    throw new Error(
      `cwd is required for the ${toolName} tool. Please provide cwd in BCPClientOptions or override the ${toolName} tool.`,
    )
  }
  return cwd
}
...
async function readFiles({
  ...
  override?: NonNullable<
    Required<BCPClientOptions>['overrideTools']['read_files']
  >
  fileFilter?: FileFilter
  cwd?: string
...
async function handleToolCall({
  ...
  overrides: NonNullable<BCPClientOptions['overrideTools']>
  ...
  if (action.type === 'prompt-error') {
    ...
    if (!parsedOutput.success) {
      const message = [
        ...
        'If this issues persists, please contact support@bujicoder.com',
      ].join('\n')
      onError({ message })
      resolve({
        sessionState: initialSessionState,
