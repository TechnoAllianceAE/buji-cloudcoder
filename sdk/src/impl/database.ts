import { validateSingleAgent } from '@bcp/common/templates/agent-validation'
import { DynamicAgentTemplateSchema } from '@bcp/common/types/dynamic-agent-template'
import { getErrorObject } from '@bcp/common/util/error'
...
export async function startAgentRun(
  ...
  try {
    ...
    if (!response.ok) {
      // Gateway doesn't have agent-runs endpoint — generate a local run ID
      // so the agent can still proceed without server-side run tracking.
      logger.info({ status: response.status }, 'startAgentRun: endpoint unavailable, using local run ID')
...
export async function finishAgentRun(
  ...
  try {
    ...
    if (!response.ok) {
      // Silently skip if endpoint unavailable (enterprise gateway)
      return
    }
  } catch (error) {
    logger.error(
