...
import { isFreeMode } from '@bcp/common/constants/free-agents'
import { models, PROFIT_MARGIN } from '@bcp/common/old-constants'
import { buildArray } from '@bcp/common/util/array'
import { normalizeProviderRequestBodyForCacheDebug } from '@bcp/common/util/cache-debug'
import { getErrorObject, promptAborted, promptSuccess } from '@bcp/common/util/error'
import { convertCbToModelMessages } from '@bcp/common/util/messages'
import { isExplicitlyDefinedModel } from '@bcp/common/util/model-utils'
import { StopSequenceHandler } from '@bcp/common/util/stop-sequence'
import {
  streamText,
  generateText,
  generateObject,
...
import type { OpenRouterProviderRoutingOptions } from '@bcp/common/types/agent-template'
import type {
  PromptAiSdkFn,
  PromptAiSdkStreamFn,
  PromptAiSdkStructuredInput,
  PromptAiSdkStructuredOutput,
} from '@bcp/common/types/contracts/llm'
import type { ParamsOf } from '@bcp/common/types/function-params'
import type { JSONObject } from '@bcp/common/types/json'
...
// Usage accounting type for OpenRouter/BCP backend responses
...
export async function* promptAiSdkStream(
  ...
  console.error('[bcp-debug] promptAiSdkStream: getModelForRequest start', { model: params.model, costMode: params.costMode })
  ...
  console.error('[bcp-debug] promptAiSdkStream: getModelForRequest done, calling streamText')
  ...
  const response = streamText({
    ...
    // When using Claude OAuth, disable retries so we can immediately fall back to BCP
    // backend on rate limit errors instead of retrying 4 times first
    ...((isClaudeOAuth || isChatGptOAuth) && { maxRetries: 0 }),
    // For ChatGPT OAuth direct, don't send BCP metadata/provider options to OpenAI
    ...(isChatGptOAuth
      ? {}
      : {
  ...
  for await (const chunkValue of response.fullStream) {
    ...
    if (chunkValue.type === 'error') {
      ...
      if (
        ...
        logger.info(
          { error: getErrorObject(chunkValue.error) },
          'Claude OAuth rate limited during stream, falling back to BCP backend',
        )
      ...
      if (chatGptErrorPolicy === 'fallback-rate-limit') {
        ...
        // In free mode, don't fall back to BCP backend — fail instead
      ...
      if (
        ...
        // Refresh failed or already retried — fall back to BCP backend
        logger.info({ model: requestedModel }, 'Claude OAuth token refresh unsuccessful, falling back to BCP backend')
        if (params.onClaudeOAuthStatusChange) {
          params.onClaudeOAuthStatusChange(false)
        }
        const fallbackResult = yield* promptAiSdkStream({
  ...
  if (!isClaudeOAuth && !isChatGptOAuth) {
    ...
    if (providerMetadata.bcp) {
      if (providerMetadata.bcp.usage) {
        const openrouterUsage = providerMetadata.bcp
...
export async function promptAiSdk(
  ...
  const modelParams: ModelRequestParams = {
    apiKey: params.apiKey,
    model: params.model,
    skipClaudeOAuth: true, // Always use BCP backend for non-streaming
    skipChatGptOAuth: true, // Always use BCP backend for non-streaming
  }
