 ...
 * - Default: Requests through BCP backend (which routes to OpenRouter)
 */

import path from 'path'

import { createAnthropic } from '@ai-sdk/anthropic'
import { BYOK_OPENROUTER_HEADER } from '@bcp/common/constants/byok'
...
import {
  ...
  toOpenAIModelId,
} from '@bcp/common/constants/chatgpt-oauth'
import {
  ...
  isClaudeModel,
  toAnthropicModelId,
} from '@bcp/common/constants/claude-oauth'
import {
  OpenAICompatibleChatLanguageModel,
  VERSION,
} from '@bcp/internal/openai-compatible/index'
 ...
 * and use BCP backend until the reset time.
...
export interface ModelRequestParams {
  /** BCP API key for backend authentication */
  apiKey: string
  /** Model ID (OpenRouter format, e.g., "anthropic/claude-sonnet-4") */
  model: string
  /** If true, skip Claude OAuth and use BCP backend (for fallback after rate limit) */
  skipClaudeOAuth?: boolean
  /** If true, skip ChatGPT OAuth and use BCP backend (for fallback after rate limit) */
 ...
 * returns an Anthropic direct model. Otherwise, returns the BCP backend model.
 * 
...
export async function getModelForRequest(params: ModelRequestParams): Promise<ModelResult> {
  ...
  if (
    ...
    // the BCP backend — bcp should only use the direct OpenAI route or fail.
  ...
  // Default: use BCP backend
  return {
    model: createBCPBackendModel(apiKey, model),
    isClaudeOAuth: false,
    isChatGptOAuth: false,
  }
}
...
/**
 * Create a model that routes through the BCP backend.
 * This is the existing behavior - requests go to BCP backend which forwards to OpenRouter.
 */
function createBCPBackendModel(
  apiKey: string,
  model: string,
): LanguageModel {
  ...
  return new OpenAICompatibleChatLanguageModel(model, {
    provider: 'codebuff',
    url: () => new URL('/v1/chat/completions', WEBSITE_URL).toString(),
    headers: () => ({
