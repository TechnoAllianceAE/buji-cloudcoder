 ...
 * Priority:
 * 1. BCP_GIT_BASH_PATH environment variable (user override)
...
function findWindowsBash(env: NodeJS.ProcessEnv): string | null {
  ...
  const customPath = env.BCP_GIT_BASH_PATH
...
2. Use WSL (Windows Subsystem for Linux):
   ...
   Then run BujiCoderPlus inside WSL.

3. Set a custom bash path:
   Set the BCP_GIT_BASH_PATH environment variable to your bash.exe location.
   Example: set BCP_GIT_BASH_PATH=C:\\path\\to\\bash.exe`,
  )
}
