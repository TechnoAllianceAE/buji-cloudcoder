...
import { CLAUDE_OAUTH_CLIENT_ID } from '@bcp/common/constants/claude-oauth'
import { env } from '@bcp/common/env'
import { userSchema } from '@bcp/common/util/credentials'
...
import type { User } from '@bcp/common/util/credentials'
 ...
 * Contains both BCP user credentials and Claude OAuth credentials.
 */
 ...
 * Prefers ~/.config/bcp{suffix}, falls back to legacy ~/.config/manicode{suffix} if it exists.
 */
export const getConfigDir = (clientEnv: ClientEnv = env): string => {
  const envSuffix =
    ...
    clientEnv.NEXT_PUBLIC_CB_ENVIRONMENT !== 'prod'
      ? `-${clientEnv.NEXT_PUBLIC_CB_ENVIRONMENT}`
      : ''
  const bcpDir = path.join(os.homedir(), '.config', `bcp${envSuffix}`)
  const legacyDir = path.join(os.homedir(), '.config', `manicode${envSuffix}`)
  // Use new path if it exists, or if legacy doesn't exist (fresh install)
  if (fs.existsSync(bcpDir) || !fs.existsSync(legacyDir)) {
    return bcpDir
  }
  return legacyDir
}
...
export const getChatGptOAuthCredentials = (
  ...
  // 2. BCP's own stored credentials
  const credentialsPath = getCredentialsPath(clientEnv)
  if (fs.existsSync(credentialsPath)) {
    try {
      const credentialsFile = fs.readFileSync(credentialsPath, 'utf8')
