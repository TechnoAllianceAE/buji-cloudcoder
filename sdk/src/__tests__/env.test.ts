import { describe, test, expect, afterEach } from '@bcp/common/testing/vitest-compat'
...
describe('sdk/env', () => {
  describe('getSdkEnv', () => {
    ...
    test('returns current process.env values for BCP_RG_PATH', () => {
      process.env.BCP_RG_PATH = '/path/to/rg'
      const env = getSdkEnv()
      expect(env.BCP_RG_PATH).toBe('/path/to/rg')
    })

    test('returns current process.env values for BCP_WASM_DIR', () => {
      process.env.BCP_WASM_DIR = '/path/to/wasm'
      const env = getSdkEnv()
      expect(env.BCP_WASM_DIR).toBe('/path/to/wasm')
    })
    ...
    test('returns undefined for unset env vars', () => {
      delete process.env.BCP_RG_PATH
      delete process.env.BCP_WASM_DIR
      const env = getSdkEnv()
      expect(env.BCP_RG_PATH).toBeUndefined()
      expect(env.BCP_WASM_DIR).toBeUndefined()
    })

    test('returns a snapshot that does not change when process.env changes', () => {
      process.env.BCP_RG_PATH = '/original/path'
      const env = getSdkEnv()
      process.env.BCP_RG_PATH = '/new/path'
      expect(env.BCP_RG_PATH).toBe('/original/path')
    })
  })
  ...
  describe('createTestSdkEnv', () => {
    ...
    test('allows overriding SDK-specific values', () => {
      const env = createTestSdkEnv({
        BCP_RG_PATH: '/custom/rg',
        BCP_WASM_DIR: '/custom/wasm',
      })
      expect(env.BCP_RG_PATH).toBe('/custom/rg')
      expect(env.BCP_WASM_DIR).toBe('/custom/wasm')
  ...
  describe('getChatGptOAuthTokenFromEnv', () => {
    ...
    test('returns undefined when token env var is unset', () => {
      delete process.env.BCP_CHATGPT_OAUTH_TOKEN
      expect(getChatGptOAuthTokenFromEnv()).toBeUndefined()
    })

    test('returns token from BCP_CHATGPT_OAUTH_TOKEN', () => {
      process.env.BCP_CHATGPT_OAUTH_TOKEN = 'chatgpt-oauth-token'
