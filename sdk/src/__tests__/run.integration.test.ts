...
import type { AgentOutput } from '@bcp/common/types/session-state'
...
describe('Prompt Caching', () => {
  it(
    ...
    async () => {
      if (!apiKey) {
        console.log(
          'Skipping prompt caching integration test: set BCP_API_KEY to run.\n' +
            'Example: BCP_API_KEY=your-key pnpm test src/__tests__/run.integration.test.ts',
        )
        return
      }
  ...
  it(
    ...
    async () => {
      ...
      try {
        ...
        const client = new BCPClient({ apiKey, cwd: process.cwd() })
