...
## Overview
Implement an **experimental, default-disabled** ChatGPT subscription OAuth feature that allows the local CLI to route eligible OpenAI-model **streaming** requests directly to OpenAI instead of BCP backend routing, mirroring the prior Claude OAuth architecture pattern.
...
12. Fallback policy:
   - Rate-limit/overload classification: auto-fallback to BCP backend.
...
13. Successful direct ChatGPT OAuth requests do **not** consume BCP credits.
...
2. Remove provider-specific/non-OpenAI fields (e.g., BCP metadata/provider routing payloads).
...
8. Successful direct requests skip BCP credit accounting path.
