...
- The SDK-driven harness made it straightforward to collect full event streams, stream chunks, structured outputs, and tmux capture paths for repeated `bcp-local-cli` runs.
- The baseline runs clearly exposed behavior patterns instead of relying on intuition.
- The BujiCoderPlus CLI itself was capable and informative during implementation-oriented runs; most inefficiency came from the tester agent's workflow rather than the CLI under test.

## What was tricky

- The `bcp-local-cli` agent uses only `run_terminal_command`, `add_message`, and `set_output`, so all tester intelligence has to come from prompt/instruction quality rather than richer tooling.
- Long BujiCoderPlus CLI responses live in a scrollable viewport. The tester spent many extra steps trying to recover hidden content even when the visible portion already contained enough evidence.
...
- The tester lacked BujiCoderPlus-specific guidance about:
...
- Adding BujiCoderPlus-specific flow guidance improved follow-up quality and reduced exploratory key usage.
...
- Added BujiCoderPlus-specific testing guidance covering ready state, smoke-test flow, implementation-test flow, long-response behavior, and session continuity expectations.
