...
1. Build an SDK-driven analysis harness for the CLI tester runs.
   - Add a reproducible script or test helper that runs `bcp-local-cli` through the SDK with `handleEvent` and `handleStreamChunk` collection.
...
2. Execute baseline mixed-scenario runs and document findings.
   ...
   - Record concrete inefficiencies, wasted actions, and missing BujiCoderPlus-CLI knowledge to drive the prompt/template changes.
...
4. Improve shared CLI tester orchestration and the concrete `bcp-local-cli` agent.
   - Update `agents/lib/create-cli-agent.ts` if shared orchestration behavior needs refinement.
   - Update `agents/bcp-local-cli.ts` with BujiCoderPlus-CLI-specific knowledge and workflow refinements informed by baseline evidence.
...
5. Add or update validation coverage.
   ...
   - At minimum, verify the agents layer still typechecks and that `claude-code-cli`, `codex-cli`, `gemini-cli`, and `bcp-local-cli` still satisfy shared construction/schema expectations.
...
- The requested `cli-ui-tester` name does not exist directly in the repo, so the harness must target the correct concrete agent (`bcp-local-cli`) and shared template layer consistently.
