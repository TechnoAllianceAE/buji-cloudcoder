...
- From monorepo root, run workspace scripts as `pnpm --dir <workspace> run <script>`; use `pnpm --dir <workspace> run <script>` for workspace commands. (from agents/sessions/03-03-0909-add-console-log)
...
- When static code analysis and tracing through the codebase isn't enough to find a bug, add targeted logging to the suspected code path, reproduce the issue live (e.g. via the bcp-local-cli tmux agent), and inspect the structured logs in `debug/web.jsonl`.
