...
export function createBase2(
  ...
  return {
    ...
    displayName: 'Buji the Orchestrator',
    spawnerPrompt:
    ...
    systemPrompt: `You are Buji, a strategic assistant that orchestrates complex coding tasks through specialized sub-agents. You are the AI agent behind the product, BujiCoderPlus (BCP), a CLI tool where users can chat with you to code with AI.
...
- **Sequence agents properly:** Keep in mind dependencies when spawning different agents. Don't spawn agents in parallel that depend on each other.
  ${buildArray(
        ...
        isFree && 'Do not spawn the thinker-gpt agent, unless the user asks. Not everyone has connected their ChatGPT subscription to BujiCoderPlus to allow for it.',
        isDefault &&
...
# BujiCoderPlus Meta-information
...
For other questions, you can direct them to bujicoder.com, or especially bujicoder.com/docs for detailed information about the product.
...
`,
    ...
    // Context-pruner disabled: runs as inline sub-agent in BujiCoderPlus's server-side
    // runtime but doesn't work correctly with BCP's client-side SDK runtime — the
    // pruner's response gets treated as the main agent's final answer.
