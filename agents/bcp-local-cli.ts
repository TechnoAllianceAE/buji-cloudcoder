...
const baseDefinition = createCliAgent({
  id: 'bcp-local-cli',
  displayName: 'BCP Local CLI',
  cliName: 'BujiCoderPlus',
  shortName: 'bcp-local',
  startCommand: 'pnpm --dir cli run dev',
  permissionNote:
    'No permission flags needed for BujiCoderPlus local dev server.',
  model: 'anthropic/claude-opus-4.6',
  skipPrepPhase: true,
  cliSpecificDocs: `## BujiCoderPlus CLI Specific Guidance

- The ready state is the BujiCoderPlus banner, working directory, and bordered input box with the agent selector.
...
- Long BujiCoderPlus responses live in a scrollable viewport. If the bottom of the answer already shows the core recommendation, do not spend many extra steps trying to reconstruct every hidden line.
...
- If the current session becomes clearly unusable, report that failure; do not silently start a replacement session and continue as though nothing happened.`,
  spawnerPromptExtras: `**Purpose:** E2E visual testing of the BujiCoderPlus CLI itself. This agent starts a local dev BujiCoderPlus CLI instance and interacts with it to verify UI behavior.
...
**How it works:** Starts \`pnpm --dir cli run dev\` in tmux, then you send prompts/commands to the CLI and capture the visual output. Unit tests and typechecks cannot catch layout bugs, rendering issues, or visual regressions - this agent captures real terminal output including colors and layout.`,
})
...
const definition: AgentDefinition = {
  ...
  handleSteps: function* ({ prompt, params, logger }) {
    const START_COMMAND = 'pnpm --dir cli run dev'
    const CLI_NAME = 'BujiCoderPlus'
