...
import { BCPClient, getUserCredentials } from '@bcp/sdk'
import { beforeAll, describe, expect, it } from '@bcp/common/testing/vitest-compat'
...
describe('Base Deep Agent Integration', () => {
  ...
  slowIt(
    'works on a shallow-cloned bcp repo for a commit-inspired focused task',
    async () => {
      const apiKey = getApiKeyOrSkip()
      if (!apiKey) return
