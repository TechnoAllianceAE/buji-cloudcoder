...
import { initTreeSitterForNode, treeSitterAvailable } from './init-node'
...
function resolveWasmPath(wasmFileName: string): string {
  ...
  if (customWasmDirPath) {
    const p = path.join(customWasmDirPath, wasmFileName)
    if (fs.existsSync(p)) return p
  }

  // Try environment variable override
  const envWasmDir = process.env.BCP_WASM_DIR
  if (envWasmDir) {
    const p = path.join(envWasmDir, wasmFileName)
    if (fs.existsSync(p)) return p
  }

  // Get the directory of this module (may be virtual in compiled binaries)
  const moduleDir = (() => {
  ...
  // Directory next to the actual executable on disk (for compiled Node bundles)
  const execDir = path.dirname(process.execPath)

  const possiblePaths = [
    // Next to the executable (compiled binary on Windows/macOS/Linux)
    path.join(execDir, 'wasm', wasmFileName),
    path.join(execDir, wasmFileName),
    // Shared WASM directory (bundled SDK)
    path.join(moduleDir, '..', 'wasm', wasmFileName),
    // WASM files in the same directory as this module
    path.join(moduleDir, 'wasm', wasmFileName),
  ...
  // Default fallback — let Language.load() report the error
...
export async function createLanguageConfig(
  ...
  if (!cfg.parser) {
    try {
      ...
      // When loaded through a runtime that returns paths, the queryText is a path to the file, not the content of the file.
...
export async function getLanguageConfig(
  ...
  // If tree-sitter WASM failed to load, skip parsing entirely
  if (!treeSitterAvailable) return undefined
  try {
