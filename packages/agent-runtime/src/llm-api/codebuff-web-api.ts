import { withTimeout } from '@bcp/common/util/promise'
...
const callBCPV1 = async (params: {
  ...
  const baseUrl = params.baseUrl ?? env.clientEnv.NEXT_PUBLIC_BCP_APP_URL
  const apiKey = params.apiKey ?? env.ciEnv.BCP_API_KEY

  if (!baseUrl || !apiKey) {
    return { error: 'Missing BCP base URL or API key' }
  }
