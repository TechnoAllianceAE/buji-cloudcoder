import { validateAgents } from '@bcp/common/templates/agent-validation'
import { parsePublishedAgentId } from '@bcp/common/util/agent-id-parsing'
import { DEFAULT_ORG_PREFIX } from '@bcp/common/util/agent-name-normalization'
...
export async function getAgentTemplate(
  ...
  if (!parsed) {
    // If agentId doesn't parse as publisher/agent format, try as {defaultOrg}/agentId
    const defaultOrgParsed = parsePublishedAgentId(
      `${DEFAULT_ORG_PREFIX}${agentId}`,
    )
    if (defaultOrgParsed) {
      const dbAgent = await fetchAgentFromDatabase({
        ...params,
        parsedAgentId: defaultOrgParsed,
      })
      if (dbAgent) {
