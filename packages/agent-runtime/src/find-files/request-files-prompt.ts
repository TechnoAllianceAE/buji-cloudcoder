...
} from '@bcp/common/old-constants'
import { getAllFilePaths } from '@bcp/common/project-file-tree'
import { isAbortError, unwrapPromptResult } from '@bcp/common/util/error'
...
async function getRelevantFiles(
  ...
  let bcpMessages = [systemMessage(system), ...messagesWithPrompt]

  // Converts assistant messages to user messages for finetuned model
  bcpMessages = bcpMessages
    .map((msg, i) => {
      if (msg.role === 'assistant' && i !== bcpMessages.length - 1) {
