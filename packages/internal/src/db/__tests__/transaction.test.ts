import * as analyticsModule from '@bcp/common/analytics'
import { AnalyticsEvent } from '@bcp/common/constants/analytics-events'
import { createPostgresError } from '@bcp/common/testing/errors'
import { afterEach, beforeEach, describe, expect, it, mock, spyOn } from '@bcp/common/testing/vitest-compat'
...
type TransactionCallback = Parameters<typeof dbModule.db.transaction>[0]
type TransactionOptions = Parameters<typeof dbModule.db.transaction>[1]
...
describe('withSerializableTransaction', () => {
  ...
  describe('successful execution', () => {
    ...
    it('should pass serializable isolation level to transaction', async () => {
      transactionSpy = spyOn(dbModule.db, 'transaction').mockImplementation(
        async (callback: TransactionCallback, options: TransactionOptions) => {
