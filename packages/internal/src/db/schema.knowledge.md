...
1. Docker running
2. Run: `pnpm --dir common run db:start`
...
3. **Use `exec` runner**: All commands must use `pnpm` to load environment variables
4. Commands: Start Docker → `pnpm --dir common run db:start` → schema operations
